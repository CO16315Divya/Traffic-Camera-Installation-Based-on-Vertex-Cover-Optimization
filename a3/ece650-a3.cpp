...
using namespace std;
int cInputProcessing(pid_t p1 ,pid_t p2 ,pid_t p3 ,pid_t p4,pid_t dPid)
{
    while(!std::cin.eof()){
    ...
    if(pIdStreetGenerator == 0){
        ...
        if( pIdGraphGenerator != 0){
            ...
            if (pIdterminator == 0) {
                ...
                close(Pipe2[1]);
                close(Pipe2[0]);
            input=cInputProcessing(pIdGraphGenerator,pIdStreetGenerator,pIdRouteFinder,pIdterminator,pIdterminator);
            }
            pid_t n = wait(NULL);
